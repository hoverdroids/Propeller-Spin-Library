...
======================================================================
   ...
   File         (HoverDroids) SerialPorts 512Byte x12.spin
   Author       Chris Sprague
   E-mail       HoverDroids@gmail.com
   Website      HoverDroids.com

   Brief Description:
     Provide full duplex serial communications for 1 to 12 serial ports, in a more
     or less transparent way, so you have COM0 to COM11 to deal with instead.
...
----------------------------------------------------------------------
  ...
  This differs from REF1 in formatting and OBJ reference naming. Use any of the
  following objects for a different number of ports and buffer sizes (as well as
  # of cogs required):
...
----------------------------------------------------------------------
  ...
  OBJ
  terminal: "(HoverDroids) SerialPorts 512Byte x12"
...
----------------------------------------------------------------------
Notes from REF1: TwelveSerialPorts512
----------------------------------------------------------------------
...
OBJ
  com0:"(HoverDroids) SerialPorts 512Byte Banks 0-3"
  com1:"(HoverDroids) SerialPorts 512Byte Banks 4-7"
  com2:"(HoverDroids) SerialPorts 512Byte Banks 8-11"
...
{ Method Info in (HoverDroids) SerialPorts PByte Banks M-N }
  ...
  return com2.str((port & 3),strAddr)
...
{ Method Info in (HoverDroids) SerialPorts PByte Banks M-N }
  ...
  return com2.strln((port & 3),strAddr)
...
{ Method Info in (HoverDroids) SerialPorts PByte Banks M-N }
  bb := (port >> 2)
  if bb == 0
    return com0.decx((port & 3),value, digits)
  if bb == 1
    return com1.decx((port & 3),value, digits)
  return com2.decx((port & 3),value, digits)

PUB decl(port,value,digits,flag) | i, x  'REF2
{ Method Info in (HoverDroids) SerialPorts PByte Banks M-N }
  bb := (port >> 2)
  if bb == 0
    return com0.decl((port & 3),value, digits, flag)
  if bb == 1
    return com1.decl((port & 3),value, digits, flag)
  return com2.decl((port & 3),value, digits, flag)
...
{ Method Info in (HoverDroids) SerialPorts PByte Banks M-N }
  ...
  return com2.ihex((port & 3),value,digits)
...
{ Method Info in (HoverDroids) SerialPorts PByte Banks M-N }
  ...
  return com2.padchar((port & 3), count, txbyte)
...
{ Method Info in (HoverDroids) SerialPorts PByte Banks M-N }
  ...
  return com2.ibin((port & 3),value,digits)
...
{ Method Info in (HoverDroids) SerialPorts PByte Banks M-N }
  ...
  return com2.putc((port & 3),txbyte)
