...
pub main | i,j,k
  ...
  debug.Init
  debug.AddPort(3,31,30,UARTSIN#PINNOTUSED,UARTSIN#PINNOTUSED,UARTSIN#DEFAULTTHRESHOLD, {
...
'must start tx before rx, otherwise pins are not in correct state       
  ...
  debug.str(3,string("serial test4",13))
  ...
  repeat
    ...
    repeat j from 0 to 2                                'for each port
      debug.dec(3,j)                                    'print port we are checking
      debug.tx(3,":")
      debug.dec(3,i)                                    'print expected number
      debug.tx(3,":")
      k := uartsin.rxcheck(j)                           'should have something
      repeat while k <> -1
        debug.dec(3,k)                                  'print what we got
        debug.tx(3," ")
        k := uartsin.rxcheck(j)                         'see if anything else (shouldn't be)
      debug.tx(3,13)                                    'next line for next port
    i++
