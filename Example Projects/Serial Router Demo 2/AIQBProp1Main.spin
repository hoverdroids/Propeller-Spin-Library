...
loaded onto the P2AB.  P2ABpassthrough wirelessly communicates user input from a PC
terminal to Prop1. Also, this file disects the message type and message value and then displays the message
number and value on the two 8 segment displays while deciding whether to pass the data to the ASD (in the future),
pass the command to Propeller2, or use the command to change the AIQB state.

Messages coded:
state,0,0,0,0  will not change state; will result in state being displayed to screen
state,A,B,C,1  will result in a state change when any/all A/B/C are 0;A=1 turns on auxDC;B=1 turns on secondaryPwr;C=1 turns on charger
...
Baud rate may differ between units though FullDuplexSerial can buffer only 16 bytes.This will eventually be problematic since
our system will transfer more data than that.So, eventually, I will modify FDSP to handle 100 bytes, which is the max number
of bytes per Xbee transmission, accordin to the XBee manual.

Also, the default baud for the Xbee is 9600.  This means that it will take rougly 16 ms to transfer each command-based on Xbee
tutorial estimation of time requirements. Again, this is not acceptable long term and will be set higher-the standard max is
115200 but Xbee can still go higher.  For now, 9600 works for testing until we reach the point where it doesn't
...
CON
  ...
  'Set pins and baud rate for Prop2Prop comms
  P1toP2_Tx = 8 ' Prop1 Tx to prop2 Rx
  P1toP2_Rx = 9 'Prop1 RX from prop2 TX
  P1toP2_baud= 115200
...
Pub Start
  aiqbP1Init              'initialize AIQB and wait for a number from the user
  ...
  repeat 
    ...
    if strcomp(@messageType,string("state"))
      state
    elseif strcomp(@messageType,string("drive"))
      commandPass
...
Pri aiqbP1Init
  {--Initialize IO board---}'this isn't necessary
  index:=%10000000
  DIOB.Main(index)

  {--Initialize all comms lines----}
  XB.start(XB_Rx, XB_Tx, 0, XB_Baud)                    ' Initialize XBee comms
  XB.rxFlush  'Empty buffer for new data
  waitcnt(clkfreq+cnt)
  
  P2PComms.start(P1toP2_RX,P1toP2_TX,0,115200)          'Initialize Prop2Prop comms
  P2PComms.rxFlush
  waitcnt(clkfreq+cnt)
  ...
  XB.str(string("Starting communications",13))
  XB.str(string(13))
  XB.str(string("Number of Cogs:"))
  XB.dec(cogCounter.free_cogs)
  XB.str(string(13))

  {--Wait for number from user before starting; without this, the prop responds unpredicatably; need to fix}
...
{{this code still needs to read the AC presence and battery level and then transmit that info
  ...
  direction:=values[6]          'REALLY NEED TO MODIFY THIS BASED ON WHERE THE COMMAS/ZEROS are in the values array
                                'THEN UPDATE  the values so that they can only be 0 or 1 so that the state read returns valid results
                                'then use this procedure in prop2 to sort out drive commands                                                                                   '
...
Pri commandPass
{Prop1 will not act on commands that are processed by prop2; it will just pass the message to Prop2}
 if secondaryPower=="1"                             'Prop1 will not pass the wheel rotation commands unless the power is on 
  P2PComms.str(@messageNew)
  P2PComms.str(string(13))                                                   
Pri timeout(secTimeout)  'secTimeout is the number of seconds to wait
  'seconds:=0
  dT := clkfreq*secTimeout
  T := cnt
  'repeat until (seconds>=secTimeout)
    T:=T+dT
    waitcnt(T)
    'seconds:=seconds+1
    'XB.str(string("seconds: ",13))
    'XB.dec(seconds)
    'XB.str(string(13))
  XB.str(string("good exit",13))
  result:=1  
Dat
