...
con
globaldebug = false ' verbosity goes here!
OBJ
...
shell byte "tcp:65534",0  ' shell:",0 ' "shell:exec logcat",0
...
repeat
  ...
  if (status == ADB_CLOSED)
    ...
    WriteMessage(A_OPEN,localID,0,@shell,strsize(@shell)+1,true)  ' send request to open shell
    term.char(13)
    status:=ADB_OPENING
