...
'' setup transmit cog
  ...
  seed := 200                                    ' make sure it(rand num gen seed value) is the same as rx cog    
                                                        'the above line only
  REPEAT
    REPEAT i FROM 0 TO constant(PCTX#BUFFER_SIZE - 1)
      long[tx_buff][i] := 200                        ' fill buffer with the forward random value;long[tx_buff][i] says to set the long at the tx_buff address to the random value
